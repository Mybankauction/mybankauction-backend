...
function replaceAndEncodeDateInURL(reqUrl) {
  ...
  const localURLPrefix =
    'https://mybankauction-backend.vercel.app/api/zoho?endpoint='
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          url: zohoUrl,
          url: zohoUrl,
          endpointWithQuery: endpointWithQuery,
          endpoint: endpoint,
          criteria: criteria,
          reqUrl: reqUrl,
        },
    ...
    if (response.status !== 204) {
      try {
        ...
        return NextResponse.json(
          { error: 'Failed to parse JSON response', url: zohoUrl },
          { status: 500 }
        )
      }
    }
